...
Means_Stds_Of_AllData<-cbind(allData[1:2],allData[grepl("mean|std",names(allData))])
...
Grouped_Means<-group_by(Means_Stds_Of_AllData,Activity,Subject)%>%summarize_each(funs(mean))
